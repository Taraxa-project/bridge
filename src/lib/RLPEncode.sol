...
library RLPEncode {
    ...
    /**
     * @dev RLP encodes a list bytes
     * @param self The list of bytes elements
     * @return The RLP encoded list of items in bytes.
     */
    function encodeBytesList(bytes[] memory self) internal pure returns (bytes memory) {
        for (uint256 i = 0; i < self.length; i++) {
            self[i] = encodeBytes(self[i]);
        }
        bytes memory list = flatten(self);
        return concat(encodeLength(list.length, 192), list);
    }
